...
export class QuarantineHouseholdIfOneMemberIll extends Policy {
    ...
    protected CanPeopleMeetInEnvironmentInternal(person1: Person, person2: Person, model: Model, environment: Environment): boolean {
        return !model.Households.Households.get(person1.HouseholdIndex).Members.any(a => [Status.MildlyIll, Status.SeriouslyIll].indexOf(a.Disease.Status) > 1 || (a.Disease.Status == Status.Recovered && a.Disease.Time.Day < 7));
    }
}
